...
// ピン留め(ライブ再計算)とは別に、「保存する」ボタンを押した時点のルートをそのまま固定して
// 後から見返せるようにする機能。[{ id, label, savedAt, targetPalId, route, requiredIds, ownedIds }]
let savedSnapshots = [];
...
function loadSnapshots() {
  const saved = localStorage.getItem(LS_KEYS.snapshots);
  if (saved) {
    try {
      savedSnapshots = JSON.parse(saved);
    } catch (e) {
      savedSnapshots = [];
    }
  }
...
function saveSnapshotsToStorage() {
  localStorage.setItem(LS_KEYS.snapshots, JSON.stringify(savedSnapshots));
}
...
function bindCarouselEvents() {
  ...
  carousel.addEventListener("click", (e) => {
    ...
    const saveBtn = e.target.closest(".save-snapshot-btn[data-save-index]");
    if (saveBtn) { saveSnapshot(Number(saveBtn.dataset.saveIndex)); return; }
  });
...
// 「保存する」ボタン。lastSlides[index]の内容(targetPal/route/ownedIdSet/requiredIds)をその時点の
// 値としてそのままコピーし、savedSnapshotsに追加する。ピン留めと違い、以後は所持パルや設定を
// 変えても再計算されない(=固定されたスナップショット)。
function saveSnapshot(index) {
  const h = lastSlides[index];
  if (!h || h.message || !h.route || !h.route.found || h.route.steps.length === 0) return;

  const snap = {
    id: Date.now(),
    label: `${h.targetPal.name}(${new Date().toLocaleString("ja-JP")})`,
    savedAt: Date.now(),
    targetPalId: h.targetPal.id,
    route: h.route,
    requiredIds: [...(h.requiredIds || [])],
    ownedIds: h.ownedIdSet ? [...h.ownedIdSet] : []
  };
  savedSnapshots = [snap, ...savedSnapshots];
  saveSnapshotsToStorage();
  renderSavedSnapshots();

  const btn = document.querySelector(`.save-snapshot-btn[data-save-index="${index}"]`);
  if (btn) {
    const original = btn.dataset.originalLabel || btn.textContent;
    btn.dataset.originalLabel = original;
  ...
  document.getElementById("savedSnapshotsList").scrollIntoView({ behavior: "smooth", block: "nearest" });
}
...
function renameSnapshot(id, newLabel) {
  const snap = savedSnapshots.find(s => s.id === id);
  if (!snap) return;
  snap.label = newLabel.trim() || snap.label;
  saveSnapshotsToStorage();
}

// 保存済みスナップショット1件分のHTML。buildSlideHtmlと似た見た目だが、固定データを描画するだけの
// 読み取り専用表示(ピン留め・除外・代替組み合わせ等のライブ機能は持たない)。
function buildSnapshotHtml(snap) {
  const targetPal = PALS.find(p => p.id === snap.targetPalId);
  if (!targetPal) return "";
  const ownedIdSet = new Set(snap.ownedIds || []);
  const reqIds = snap.requiredIds || [];
  ...
  const palTag = (pal) => {
    ...
    const colorStyle = color ? ` style="border-color:${color}; color:${color};"` : "";
    const icon = `<img class="pal-tag-icon" src="images/pal-${pal.paldexId.toLowerCase()}.png" alt="" loading="lazy" onerror="this.style.display='none'">`;
    return `<span class="pal-tag ${ownedClass}"${colorStyle}>${icon}${pal.name}</span>`;
  };
  ...
  const isHatchMode = route.totalHatchHours !== undefined;
  const displayHatchHours = isHatchMode ? route.totalHatchHours : computeCriticalPathHours(route.steps, ownedIdSet);
  const summaryLine = `「${targetPal.name}」まで <strong>${route.steps.length}回の配合</strong>(孵化時間合計 目安 <strong>${formatHatchHours(displayHatchHours)}</strong>)`;
  const savedAtText = new Date(snap.savedAt).toLocaleString("ja-JP");

  return `
    <div class="card" data-snapshot-id="${snap.id}">
      <div style="display:flex; justify-content:space-between; align-items:center; gap:8px; flex-wrap:wrap; margin-bottom:6px;">
        <input type="text" class="owned-search snapshot-label-input" data-snapshot-rename="${snap.id}" value="${snap.label.replace(/"/g, "&quot;")}" style="flex:1; min-width:160px; font-weight:bold;">
        <button type="button" class="secondary" data-snapshot-delete="${snap.id}">削除</button>
      </div>
      <p class="hint" style="margin-bottom:8px;">保存日時: ${savedAtText}</p>
      <p class="result-summary">${summaryLine}</p>
      <div>${stepsHtml}</div>
    </div>
  `;
...
function renderSavedSnapshots() {
  const box = document.getElementById("savedSnapshotsList");
  if (savedSnapshots.length === 0) {
    box.innerHTML = `<p class="hint">まだ保存したルートはありません。計算結果の「💾 保存する」ボタンで追加できます。</p>`;
    return;
  }
  box.innerHTML = savedSnapshots.map(buildSnapshotHtml).join("");
}

function bindSnapshotEvents() {
  const box = document.getElementById("savedSnapshotsList");
  box.addEventListener("click", (e) => {
    const delBtn = e.target.closest("[data-snapshot-delete]");
    if (delBtn) { deleteSnapshot(Number(delBtn.dataset.snapshotDelete)); return; }
  });
  box.addEventListener("change", (e) => {
    const input = e.target.closest("[data-snapshot-rename]");
    if (input) { renameSnapshot(Number(input.dataset.snapshotRename), input.value); }
  });
}
    ...
    <div class="result-slide">
      <div class="route-actions">
        ...
        <div style="display:flex; gap:8px; flex-wrap:wrap;">
          ...
          <button type="button" class="secondary save-snapshot-btn" data-save-index="${slideIndex}">💾 保存する</button>
        </div>
      </div>
